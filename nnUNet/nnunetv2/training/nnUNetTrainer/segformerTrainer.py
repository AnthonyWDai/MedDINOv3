...
class segformerTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(patch_size: tuple, 
        ...
        # Initialize model
        model = SegFormer(model_name='B2', num_classes=num_output_channels, image_size=patch_size[0])
        model.init_weights()
        return model
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.network.parameters(), self.initial_lr, weight_decay=self.weight_decay, betas = (0.9, 0.98))
...
class Decoder(nn.Module):
    def __init__(self, dims, embed_dim, num_classes):
        ...
        self.conv_up = nn.ConvTranspose2d(num_classes, num_classes, 4, 4)
    ...
    def forward(self, inputs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        c = self.dropout(c)
        return self.conv_up(self.linear_pred(c))
...
class SegFormer(nn.Module):
    def __init__(self, model_name: str = 'B0', num_classes: int = 19, image_size: int = 224) -> None:
        ...
        self.decoder = Decoder(dims, embed_dim, num_classes)
